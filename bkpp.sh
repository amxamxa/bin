...
# Based on: BKP template (cp-version)
#
# Usage:
...
_run_backup() {
    ...
    for source in "${SOURCES[@]}"; do
        ...
        # ── Create target dir (idempotent – rsync syncs incrementally) ──────────
        # No guard against existing directories: rsync -u skips files that are
        # already up-to-date, so re-running is safe and copies only new files.
        if [[ -e "$target_dir" ]]; then
            printf "%s bkp-to-desti.sh: Info: Target '%s' exists – syncing new/changed files only. %s\n" \
                "$BROWN" "$target_dir" "$RESET"
        fi
...
# Usage: source bkp-to-desti.sh && _test_bkp_functionality
################################################################################
_test_bkp_functionality() {
